...
def split_text_dd(text: str, n=100, character=" ") -> List[str]:
    """Split the text every ``n``-th occurrence of ``character``"""
    passages = []
    for passage in split_text(text, 1, "####"):
...
def split_documents(documents: dict) -> dict:
    ...
    for title, text in zip(documents["title"], documents["text"]):
        if text is not None:
            for passage in split_text_dd(text):
                titles.append(title if title is not None else "")
                texts.append(passage)
    print("###### num passage", len(texts))
...
def main(
    ...
    Path(rag_example_args.output_dir).mkdir(exist_ok=True)
    passages_path = os.path.join(rag_example_args.output_dir, "my_knowledge_dataset")
    Path(passages_path).mkdir(exist_ok=True)
    ...
    # index = faiss.IndexHNSWFlat(index_args.d, index_args.m, faiss.METRIC_INNER_PRODUCT)
    index = faiss.IndexFlatIP(index_args.d)
    dataset.add_faiss_index("embeddings", custom_index=index)

    # And save the index
    index_path = os.path.join(rag_example_args.output_dir, "my_knowledge_dataset_index.faiss")
...
if __name__ == "__main__":
    ...
    rag_example_args, processing_args, index_args = parser.parse_args_into_dataclasses()
    with TemporaryDirectory() as tmp_dir:
        rag_example_args.output_dir = rag_example_args.output_dir or tmp_dir
        main(rag_example_args, processing_args, index_args)