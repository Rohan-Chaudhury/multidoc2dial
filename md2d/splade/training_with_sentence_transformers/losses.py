...
class MultipleNegativesRankingLossSpladePair(nn.Module):
    """
        This loss expects as input a batch consisting of sentence pairs (a_1, p_1), (a_2, p_2), ..., (a_n, p_n)
        where we assume that (a_i, p_i) are positive pairs and (a_i, p_j) for i!=j negative pairs.
        For each a_i, it uses all other p_j as negative samples, i.e., for a_i, we have 1 positive example (p_i) and
        ...
        Here, n_1 is a hard negative for (a_1, p_1). The loss will use for the pair (a_i, p_i) all p_j (j!=i) and all n_j as negatives.
    """
    def __init__(self, model, scale: float = 1.0, similarity_fct = dot_score, lambda_d=0.0008, lambda_q=0.0006, pair_lambda=0.1):
        """
        :param model: SentenceTransformer model
        :param scale: Output of similarity function is multiplied by scale value
        :param similarity_fct: similarity function between sentence embeddings. By default, cos_sim. Can also be set to dot product (and then set scale to 1)
        """
        super(MultipleNegativesRankingLossSpladePair, self).__init__()
        self.model = model
        self.scale = scale
        self.similarity_fct = similarity_fct
        self.cross_entropy_loss = nn.CrossEntropyLoss()
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        reps = [self.model(sentence_feature)['sentence_embedding'] for sentence_feature in sentence_features]
        embeddings_a = reps[0]
        positive_passages = reps[1]
        embeddings_b = torch.cat(reps[1:])

        query_scores = self.similarity_fct(embeddings_a, embeddings_b) * self.scale
        labels = torch.tensor(range(len(query_scores)), dtype=torch.long, device=query_scores.device)  # Example a[i] should match with b[i]

        pair_mask = torch.eye(n=embeddings_a.shape[0], dtype=torch.bool, device=labels.device)
        
        # pair_query_scores = self.similarity_fct(embeddings_a, positive_passages) * self.scale
        # pair_passage_scores = self.similarity_fct(positive_passages, positive_passages) * self.scale
        
        # pair_passage_scores = torch.masked_fill(pair_passage_scores, pair_mask, -100.0) # remove positive passages
        # pair_passage_scores = torch.exp(pair_passage_scores)
        # pair_passage_scores = pair_passage_scores.sum(dim=1)
        
        # pair_query_scores = torch.masked_fill(pair_query_scores, ~pair_mask, -100.0) # remove irrelevant query-passage interactions
        # pair_query_scores = torch.exp(pair_query_scores)
        # pair_query_scores = pair_query_scores.sum(dim=1)

        # # pair_scores = torch.cat([pair_query_scores, pair_passage_scores])
        # # pair_labels = torch.tensor(range(len(pair_scores)), dtype=torch.long, device=pair_scores.device) 

        # pair_scores = pair_query_scores/(pair_query_scores + pair_passage_scores)
        # pair_loss = -torch.log(pair_scores)
        ...
        pair_logits = torch.cat([pair_query_scores, pair_passage_scores], dim=1)

        flops_doc = self.lambda_d*(self.FLOPS(embeddings_b))
        flops_query = self.FLOPS(embeddings_a)

        return (1-self.pair_lambda)*self.cross_entropy_loss(query_scores, labels) + self.pair_lambda*self.cross_entropy_loss(pair_logits, labels) + flops_doc + flops_query
        # return (1-self.pair_lambda)*self.cross_entropy_loss(query_scores, labels) + self.pair_lambda*pair_loss + flops_doc + flops_query
