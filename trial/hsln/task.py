...
for i in range(len(labels)):
    ...
    labels[i]=labels[i].replace('\s+', '_')
    labels[i]=labels[i].replace('\t', '_')

PUBMED_LABELS = labels
PUBMED_LABELS_PRES = labels
...
def pubmed_task(train_batch_size, max_docs, data_folder="datasets/"):
    return Task(PUBMED_TASK, PUBMED_LABELS,
                train_batch_size, 1, max_docs, short_name="multidoc2dial", labels_pres=PUBMED_LABELS_PRES,
