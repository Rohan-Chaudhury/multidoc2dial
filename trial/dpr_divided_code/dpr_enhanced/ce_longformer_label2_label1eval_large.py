...
cross_encoder_path = "/home/grads/r/rohan.chaudhury/multidoc2dial/multidoc2dial/trial/dpr_divided_code/dpr_enhanced/longformer_models/large_models/normal_loss/checkpoint-29495"
print ("Cross encoder path: ", cross_encoder_path)

cross_model = LongformerForSequenceClassification.from_pretrained(cross_encoder_path,
    ...
    for batch_idx in tqdm(range(num_batches)):
        ...
        with torch.no_grad():
            logits = cross_model(**tokenized_inputs, 
                       global_attention_mask=global_attention_masks).logits[:, 1].squeeze().detach().cpu().numpy()
...
cross_encoder_path = "/home/grads/r/rohan.chaudhury/multidoc2dial/multidoc2dial/trial/dpr_divided_code/dpr_enhanced/longformer_models/margin_loss/checkpoint-9384"
print ("Cross encoder path: ", cross_encoder_path)
...
def rerank_with_cross_encoder(cross_questions, top_10_indices, context_passages, cross_model, cross_tokenizer, k=10, batch_size=5):
    cross_model.eval()
    max_length = 1024
    num_batches = math.ceil(len(cross_questions) / batch_size)
    reranked_indices = []
