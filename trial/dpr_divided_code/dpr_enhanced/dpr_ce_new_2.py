...
class T5CrossEncoder(nn.Module):
    def __init__(self, pretrained_model_name, model_max_length, dropout_rate=0.25, num_heads=64, attention_dropout=0.15):
        super().__init__()
        ...
        self.attention = nn.MultiheadAttention(config.d_model, num_heads=num_heads, dropout=attention_dropout)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        num_heads_times_batch_size = self.num_heads 
...
def train_dpr_model(config, train_dataset, validation_dataset, cross_encoder, t5_tokenizer, device):
    ...
    print ("Let's see what happens now now now 7\n")
