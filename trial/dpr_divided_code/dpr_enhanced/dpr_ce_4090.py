...
class T5CrossEncoder(nn.Module):
    def __init__(self, pretrained_model_name, model_max_length, dropout_rate=0.1, position_dropout=0.1):
        super().__init__()
        ...
        self.attention = nn.MultiheadAttention(config.d_model, num_heads=16, dropout=0.1)
        self.classifier = self._init_classifier(2 * config.d_model)
        self.layer_norm = nn.LayerNorm(config.d_model)
        self.positional_encoding = PositionalEncoding(d_model=config.d_model, dropout=position_dropout)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        positional_encoded_state = self.positional_encoding(last_hidden_state.clone())
        ...
        num_heads_times_batch_size = 16  # replace '16' with the actual number of heads
        ...
        positional_encoded_state = positional_encoded_state.transpose(0, 1)

        # Apply multi-head self-attention
        attention_output, _ = self.attention(positional_encoded_state, 
                                             positional_encoded_state, 
                                             positional_encoded_state,
...
with open("C:\\Users\\softi\\Projects\\multidoc2dial\\Zhang\\dpr.multidoc2dial_all.structure.train.json", "r", encoding="utf-8") as f:
...
with open("C:\\Users\\softi\\Projects\\multidoc2dial\\Zhang\\dpr.multidoc2dial_all.structure.validation.json", "r", encoding="utf-8") as f:
...
def preprocess_question(question):
    return truncate_question_sequences(remove_extra_spaces(question), 256)
...
def preprocess_data(training_data):
    ...
    for item in tqdm(training_data):
        ...
        for positive_ctx in item["positive_ctxs"]:
            ...
            # For each positive context, pair it with negative contexts
            # First, from "negative_ctxs"
            for negative_ctx in item["negative_ctxs"]:
            ...
            # Then, from "hard_negative_ctxs" if it exists
            if "hard_negative_ctxs" in item:
                for hard_negative_ctx in item["hard_negative_ctxs"]:
                    negative_context = remove_extra_spaces(hard_negative_ctx["text"])
...
print ("Doing data processing now newly in 2--! \n")
...
def validate_dpr_model(config, validation_dataset, cross_encoder, t5_tokenizer, device, best_val_loss):
    ...
    if avg_val_loss < best_val_loss:
        ...
        directory_to_save="C:\\Users\\softi\\Projects\\multidoc2dial\\Zhang\\output\\"+timestamp
...
def train_dpr_model(config, train_dataset, validation_dataset, cross_encoder, t5_tokenizer, device):
    ...
    print ("Let's see what happens now now now 10 10 768\n")
    max_length = config["max_length"]
    ...
    for epoch in range(epochs):
        ...
        best_val_loss = validate_dpr_model(config, validation_dataset, cross_encoder, t5_tokenizer, device, best_val_loss)
