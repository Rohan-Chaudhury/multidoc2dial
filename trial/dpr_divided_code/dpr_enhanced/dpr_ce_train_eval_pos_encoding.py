...
class T5CrossEncoder(nn.Module):
    def __init__(self, pretrained_model_name, model_max_length, dropout_rate=0.25, position_dropout=0.25):
        super().__init__()
        ...
        self.attention = nn.MultiheadAttention(config.d_model, num_heads=16, dropout=dropout_rate)
        self.classifier = self._init_classifier(2 * config.d_model)
        self.layer_norm = nn.LayerNorm(config.d_model)
        self.positional_encoding = PositionalEncoding(d_model=config.d_model, dropout=position_dropout, max_len=model_max_length)
...
print ("Doing data processing now newly now pos 1024 no lookahead --! \n")
preprocessed_data = preprocess_data(training_data, negative_weight=15)
...
def train_dpr_model(config, train_dataset, validation_dataset, cross_encoder, t5_tokenizer, device):
    ...
    print ("\n----Cross Encoder no Lookahead implemented with new scheduler----\n")
