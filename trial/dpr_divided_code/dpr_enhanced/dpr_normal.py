...
with open("/home/grads/r/rohan.chaudhury/multidoc2dial/multidoc2dial/trial/dpr_divided_code/dpr_enhanced/enhanced_dpr/training_psg_data.json", "r") as f:
    training_data = json.load(f)

# with open("/home/softlab/Documents/md2d_trial/to_zip/dpr.psg.multidoc2dial_all.structure.json", "r") as f:
#     corpus_data = json.load(f)

with open("/home/grads/r/rohan.chaudhury/multidoc2dial/multidoc2dial/trial/dpr_divided_code/dpr_enhanced/enhanced_dpr/validation_psg_data.json", "r") as f:
    validation_data = json.load(f)

with open("/home/grads/r/rohan.chaudhury/multidoc2dial/multidoc2dial/trial/dpr_divided_code/dpr_enhanced/enhanced_dpr/test_psg_data.json", "r") as f:
...
checkpoint_dpr = torch.load(checkpoint_path_dpr, map_location=device)
print ("Loss: ", checkpoint_dpr['loss'])
model.load_state_dict(checkpoint_dpr['model_state_dict'], strict=False)
...
class CombinedLoss(nn.Module):
    def __init__(self, margin=1, running_factor=0.90):
        super(CombinedLoss, self).__init__()
        self.margin = margin
        self.running_factor = running_factor
        self.running_loss_distance = 0.0
        self.running_loss_dot_product = 0.0

    def forward(self, query_vectors, positive_vectors, negative_vectors):
        distance_loss = self.compute_distance_loss(query_vectors, positive_vectors, negative_vectors)
        dot_product_loss = self.compute_dot_product_loss(query_vectors, positive_vectors, negative_vectors)

        # Update the running losses
        self.running_loss_distance = self.running_factor * self.running_loss_distance + (1 - self.running_factor) * distance_loss.item()
        self.running_loss_dot_product = self.running_factor * self.running_loss_dot_product + (1 - self.running_factor) * dot_product_loss.item()

        if self.running_loss_distance == 0.0 and self.running_loss_dot_product == 0.0:
            weight_distance = 0.5
            weight_dot_product = 0.5
        else:
            # Calculate the dynamic weights based on the running losses
            weight_distance = self.running_loss_distance / (self.running_loss_distance + self.running_loss_dot_product)
            weight_dot_product = self.running_loss_dot_product / (self.running_loss_distance + self.running_loss_dot_product)

        # Combine the losses with the dynamic weights
        loss = weight_distance * distance_loss + weight_dot_product * dot_product_loss
        return loss

    def compute_distance_loss(self, query_vectors, positive_vectors, negative_vectors):
        distances_pos = (query_vectors - positive_vectors).pow(2).sum(1)
        distances_neg = [(query_vectors - neg_vectors).pow(2).sum(1) for neg_vectors in negative_vectors]
        losses = [F.relu(distances_pos - distance_negative + self.margin) for distance_negative in distances_neg]
...
def validate(model, dataloader, loss_fn, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Validation", ncols=100):
            ...
            loss = loss_fn.compute_distance_loss(query_outputs, pos_context_outputs, neg_contexts_outputs)+loss_fn.compute_dot_product_loss(query_outputs, pos_context_outputs, neg_contexts_outputs)
