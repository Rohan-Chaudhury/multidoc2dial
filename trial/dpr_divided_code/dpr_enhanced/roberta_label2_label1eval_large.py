...
top_10_indices = search_faiss(question_embeddings, index, k=60)

cross_encoder_path = "/home/grads/r/rohan.chaudhury/multidoc2dial/multidoc2dial/trial/dpr_divided_code/dpr_enhanced/longformer_models/Roberta_large/normal_loss/checkpoint-14747"
print ("Cross encoder path: ", cross_encoder_path)

cross_model = RobertaForSequenceClassification.from_pretrained(cross_encoder_path)
cross_model= nn.DataParallel(cross_model)
cross_model.to(device)

cross_tokenizer = RobertaTokenizerFast.from_pretrained(cross_encoder_path, max_length = 512)
...
def rerank_with_cross_encoder(cross_questions, top_10_indices, context_passages, cross_model, cross_tokenizer, k=10, batch_size=2):
    cross_model.eval()
    max_length = 512
    ...
    for batch_idx in tqdm(range(num_batches)):
        ...
        with torch.no_grad():
            logits = cross_model(**tokenized_inputs).logits[:, 1].squeeze().detach().cpu().numpy()
...
recall_60 = recall_at_k(top_10_indices, positive_psg_ids, 60)

print(f"Recall@1: {recall_1:.4f}")
print(f"Recall@5: {recall_5:.4f}")
print(f"Recall@10: {recall_10:.4f}")
print(f"Recall@60: {recall_60:.4f}")
...
top_10_indices_cosine = top_10_indices
import pickle
...
recall_60 = recall_at_k(top_10_indices_cosine, positive_psg_ids, 60)
...
def preprocess_question(question):
    question = truncate_question_sequences(question, cross_tokenizer, max_question_len=128)
...
# reranked_indices_cosine_2 = rerank_with_cross_encoder(cross_questions, reranked_indices_cosine, corpus_data_list , cross_model, cross_tokenizer, k=10)
# # Calculate recall@1, recall@5, recall@10 scores
# recall_1_cosine_2 = recall_at_k(reranked_indices_cosine_2, positive_psg_ids, 1)
# recall_5_cosine_2 = recall_at_k(reranked_indices_cosine_2, positive_psg_ids, 5)
# recall_10_cosine_2 = recall_at_k(reranked_indices_cosine_2, positive_psg_ids, 10)

# print(f"Recall@1 (Cosine): {recall_1_cosine_2:.4f}")
# print(f"Recall@5 (Cosine): {recall_5_cosine_2:.4f}")
# print(f"Recall@10 (Cosine): {recall_10_cosine_2:.4f}")
...
cross_encoder_path = "/home/grads/r/rohan.chaudhury/multidoc2dial/multidoc2dial/trial/dpr_divided_code/dpr_enhanced/longformer_models/Roberta_large/margin_loss/checkpoint-6703"
print ("Cross encoder path: ", cross_encoder_path)
...
cross_model = RobertaForSequenceClassification.from_pretrained(cross_encoder_path,config=config)
cross_model= nn.DataParallel(cross_model)
cross_model.to(device)
...
def rerank_with_cross_encoder(cross_questions, top_10_indices, context_passages, cross_model, cross_tokenizer, k=10, batch_size=2):
    ...
    for batch_idx in tqdm(range(num_batches)):
        ...
        with torch.no_grad():
            logits = cross_model(**tokenized_inputs).logits.squeeze().detach().cpu().numpy()
            logits = logits.reshape(len(batch_questions), -1)
