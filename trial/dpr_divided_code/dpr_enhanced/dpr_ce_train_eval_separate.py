...
class AttentionPooling(nn.Module):
    ...
    def forward(self, hidden_states):
        attention_weights = self.softmax(self.linear(hidden_states))
...
class T5CrossEncoder(nn.Module):
    def __init__(self, pretrained_model_name, model_max_length, dropout_rate=0.2):
        ...
        self.attention_pooling = AttentionPooling(config.d_model)  # attention pooling layer
    ...
    def _init_classifier(self, d_model):
        ...
        for module in classifier:
            if isinstance(module, nn.Linear):
                module.weight.data.normal_(mean=0.0, std=0.02)  # initialize from normal distribution
        return classifier
    ...
    def forward(self, input_ids, attention_mask):
        ...
        attention_mask_for_multihead = (1.0 - attention_mask) * -1e9
        attention_mask_for_multihead = attention_mask_for_multihead.to(last_hidden_state.dtype)
        seq_len = last_hidden_state.shape[1]
        attention_mask_for_multihead = attention_mask_for_multihead.unsqueeze(1).expand(-1, seq_len, -1)
        num_heads_times_batch_size = 16
        ...
        attention_output = self.layer_norm(attention_output + last_hidden_state)
        attention_output = attention_output.transpose(0, 1)

        # replace mean and max pooling with attention pooling
        pooled_output = self.attention_pooling(attention_output)
...
print ("Doing data processing now newly now 6! \n")
preprocessed_data = preprocess_data(training_data, negative_weight=10)
...
def train_dpr_model(config, train_dataset, validation_dataset, cross_encoder, t5_tokenizer, device):
    ...
    print ("\n----Cross Encoder with Lookahead implemented with new scheduler----\n")
    ...
    cross_encoder_scheduler = get_linear_schedule_with_warmup(cross_encoder_optimizer, num_warmup_steps=config["num_warmup_steps"], num_training_steps=total_steps)
    # model.to(device)
