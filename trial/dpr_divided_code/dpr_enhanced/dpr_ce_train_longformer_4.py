...
class LongformerCrossEncoder(nn.Module):
    def __init__(self, pretrained_model_name, dropout_rate=0.2):
        ...
        self.classifier = self._init_classifier(config.hidden_size)  # Adjusting for concatenation
        self.layer_norm = nn.LayerNorm(config.hidden_size)  # Adjusting for concatenation
    ...
    def forward(self, input_ids, attention_mask, global_attention_mask):
        ...
        combined_representation = self.dropout(cls_representation)
...
longformer_cross_encoder = LongformerCrossEncoder(longformer_pretrained_model_name)
...
longformer_tokenizer = LongformerTokenizer.from_pretrained(longformer_pretrained_model_name, model_max_length=model_max_length, truncation=True)


def create_cross_encoder_global_attention_mask(input_ids, tokenizer):
    # Start with a zero mask
    global_attention_mask = torch.zeros_like(input_ids)
    
    # Set global attention on the entire first sequence (up to the first [SEP] token)
    for idx, row in enumerate(input_ids):
        sep_position = (row == tokenizer.sep_token_id).nonzero().squeeze().tolist()
        # Assuming the first sequence ends at the first [SEP] token
        global_attention_mask[idx, :sep_position[0]+1] = 1
    
    # print ("\n\n Global attention mask is \n\n", global_attention_mask[0])
    # print ("\n\n Input ids is \n\n", input_ids[0])
...
def validate_dpr_model(config, validation_dataset, cross_encoder, longformer_tokenizer, device, best_val_loss):
    ...
    for batch in val_iter:
        ...
        with torch.no_grad():
            t5_encodings = longformer_tokenizer(positive_t5_input, return_tensors="pt", padding='longest', truncation=True, max_length=config["model_max_length"])
            t5_input_ids, t5_attention_mask = t5_encodings["input_ids"].to(device), t5_encodings["attention_mask"].to(device)
            global_attention_mask = create_cross_encoder_global_attention_mask(t5_input_ids, longformer_tokenizer)
            positive_logits = cross_encoder(t5_input_ids, t5_attention_mask, global_attention_mask).squeeze()

            t5_encodings = longformer_tokenizer(negative_t5_input, return_tensors="pt", padding='longest', truncation=True, max_length=config["model_max_length"])
            t5_input_ids, t5_attention_mask = t5_encodings["input_ids"].to(device), t5_encodings["attention_mask"].to(device)
            global_attention_mask = create_cross_encoder_global_attention_mask(t5_input_ids, longformer_tokenizer)
            negative_logits = cross_encoder(t5_input_ids, t5_attention_mask, global_attention_mask).squeeze()
        ...
        # del positive_logits, negative_logits, t5_input_ids, t5_attention_mask, global_attention_mask
