...
longformer_tokenizer = LongformerTokenizer.from_pretrained(longformer_pretrained_model_name)
...
print ("Doing data processing now for longformer--! \n")
...
def validate_dpr_model(config, validation_dataset, cross_encoder, longformer_tokenizer, device, best_val_loss):
    ...
    for batch in val_iter:
        ...
        with torch.no_grad():
            t5_encodings = longformer_tokenizer(positive_t5_input, return_tensors="pt", padding='longest', truncation=False)
            ...
            t5_encodings = longformer_tokenizer(negative_t5_input, return_tensors="pt", padding='longest', truncation=False)
            t5_input_ids, t5_attention_mask = t5_encodings["input_ids"].to(device), t5_encodings["attention_mask"].to(device)
            global_attention_mask = create_cross_encoder_global_attention_mask(t5_input_ids, longformer_tokenizer)
