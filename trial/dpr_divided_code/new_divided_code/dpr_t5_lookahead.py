...
class T5CrossEncoder(nn.Module):
    def __init__(self, pretrained_model_name, model_max_length, dropout_rate=0.15):
        super().__init__()
...
print ("\n New Flan Datasets - whole full with new batch size ----\n")
...
def train_dpr_model(config, train_dataset, validation_dataset, cross_encoder, t5_tokenizer, device):
    ...
    cross_encoder_optimizer = optim.Lookahead(cross_encoder_optimizer, k=10, alpha=0.5)
    print ("\n----Cross Encoder Lookahead implemented----\n")
