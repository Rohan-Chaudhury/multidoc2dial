...
def truncate_question_sequences(question,t5_tokenizer, max_question_len=500):
    # print (question)
...
checkpoint_path_ce = "/home/grads/r/rohan.chaudhury/multidoc2dial/multidoc2dial/trial/dpr_divided_code/bert/ce_model_roberta.pt"
...
def load_saved_model(checkpoint_path_dpr, checkpoint_path_ce):
    ...
    t5_pretrained_model_name ='deepset/roberta-large-squad2'
    t5_cross_encoder = Reranker(model_name=t5_pretrained_model_name)
    t5_cross_encoder= nn.DataParallel(t5_cross_encoder)
    t5_cross_encoder.to(device)
    t5_cross_encoder.load_state_dict(checkpoint_ce, strict=False)
...
t5_pretrained_model_name = 'deepset/roberta-large-squad2'
model_max_length = 1024
t5_tokenizer = AutoTokenizer.from_pretrained(t5_pretrained_model_name)
...
def rerank_with_cross_encoder(t5_questions, top_10_indices, context_passages, cross_encoder, tokenizer, k=10, batch_size=4):
    max_length = 1024
    ...
    for batch_idx in tqdm(range(num_batches)):
        ...
        batch_questions = [truncate_question_sequences(question, tokenizer, 128) for question in batch_questions]
        t5_inputs = [[question,context_passages[top_idx]] for question, top_indices in zip(batch_questions, batch_top_indices) for top_idx in top_indices]
        ...
        # t5_encodings = tokenizer(t5_input, return_tensors="pt", padding='max_length', truncation=True, max_length=max_length)
        t5_encodings=tokenizer(t5_inputs, return_tensors='pt', padding=True, truncation=True, max_length=512)
