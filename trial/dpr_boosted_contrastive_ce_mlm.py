...
config = {
    ...
    "patience": 3,
    "temperature": 1.0,
    "lambda_mlm_question" : 0.1,
    "lambda_mlm_context" : 0.1,
    "update_rate" : 0.1
}
...
class CustomDPRQuestionEncoderWithDropout(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        return self.linear(self.dropout(self.layer_norm(pooled_output))), loss
...
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "1,2,3"  # Set this to the index of the GPU you want to use
import torch
import torch.nn as nn
...
preprocessed_data = preprocess_data(training_data[:10])
train_dataset = Dataset.from_dict(preprocessed_data)
train_dataloader = DataLoader(train_dataset, batch_size=config["batch_size"], shuffle=True)

preprocessed_validation_data = preprocess_data(validation_data[:10])
...
class CustomDataCollatorForLanguageModeling(DataCollatorForLanguageModeling):
    def __init__(self, tokenizer, mlm_probability):
        ...
        self.masking_probability = mlm_probability
    def torch_mask_tokens(self, inputs: torch.Tensor, special_tokens_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        probability_matrix = torch.full(inputs.shape, self.masking_probability, device=device)
        special_tokens_mask = special_tokens_mask.to(inputs.device)
        probability_matrix.masked_fill_(special_tokens_mask, value=0.0)
        masked_indices = torch.bernoulli(probability_matrix).bool()
        labels = torch.where(masked_indices, inputs, torch.tensor(-100).to(inputs.device))
    ...
    def torch_call(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        # Add 'attention_mask' to the output batch
...
def train_dpr_model(config, train_dataset, validation_dataset, model, cross_encoder, question_tokenizer, context_tokenizer, t5_tokenizer, device):
    ...
    running_cross_encoder_loss = 0
    running_question_mlm_loss = 0
    running_positive_mlm_loss = 0
    running_negative_mlm_loss = 0
    for epoch in range(epochs):
        ...
        for idx, batch in enumerate(train_iter):
            ...
            anchor_input_ids_list = [x.to(device) for x in anchor_input_ids]
            positive_input_ids_list = [x.to(device) for x in positive_input_ids]
            negative_input_ids_list = [x.to(device) for x in negative_input_ids]
            # Compute embeddings
            processed_batch =  data_collator(anchor_input_ids_list)
            mlm_labels_anchor = processed_batch['labels'].to(device)
            anchor_input_ids = processed_batch['input_ids'].to(device)
            anchor_attention_mask = processed_batch['attention_mask'].to(device)
            anchor_embeddings, question_mlm_loss = model.question_encoder(anchor_input_ids, anchor_attention_mask, labels=mlm_labels_anchor)

            processed_batch = data_collator(positive_input_ids_list)
            mlm_labels_positive = processed_batch['labels'].to(device)
            positive_input_ids = processed_batch['input_ids'].to(device)
            positive_attention_mask = processed_batch['attention_mask'].to(device)
            positive_embeddings, positive_mlm_loss = model.context_encoder(positive_input_ids, positive_attention_mask, labels=mlm_labels_positive)

            processed_batch = data_collator(negative_input_ids_list)
            mlm_labels_negative = processed_batch['labels'].to(device)
            negative_input_ids = processed_batch['input_ids'].to(device)
            negative_attention_mask = processed_batch['attention_mask'].to(device)
            negative_embeddings, negative_mlm_loss = model.context_encoder(negative_input_ids, negative_attention_mask, labels=mlm_labels_negative)
            ...
            running_contrastive_loss = (1 - update_rate) * running_contrastive_loss + update_rate * loss.item()
            running_cross_encoder_loss = (1 - update_rate) * running_cross_encoder_loss + update_rate * cross_encoder_loss.item()
            running_question_mlm_loss = (1 - update_rate) * running_question_mlm_loss + update_rate * question_mlm_loss.item()
            running_positive_mlm_loss = (1 - update_rate) * running_positive_mlm_loss + update_rate * positive_mlm_loss.item()
            running_negative_mlm_loss = (1 - update_rate) * running_negative_mlm_loss + update_rate * negative_mlm_loss.item()

            total_running_loss = running_contrastive_loss + running_cross_encoder_loss + running_question_mlm_loss + running_positive_mlm_loss + running_negative_mlm_loss

            adaptive_weights = [
                running_contrastive_loss / total_running_loss,
                running_cross_encoder_loss / total_running_loss,
                running_question_mlm_loss / total_running_loss,
                running_positive_mlm_loss / total_running_loss,
                running_negative_mlm_loss / total_running_loss,
            ]

        # Combine the losses

            combined_loss = sum(weight * loss for weight, loss in zip(adaptive_weights, [loss, cross_encoder_loss, question_mlm_loss, positive_mlm_loss, negative_mlm_loss]))
        ...
        for batch in val_iter:
            ...
            lambda_mlm_question = config["lambda_mlm_question"]
            lambda_mlm_context = config["lambda_mlm_context"]
            # Combine the losses
            combined_loss = (1 - alpha) * loss + alpha * cross_encoder_loss + lambda_mlm_question * question_mlm_loss + lambda_mlm_context * (positive_mlm_loss + negative_mlm_loss)
